...
def next_page():
    for i in range(1,8):  # selenium 循环翻页成功！
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into xian_Account_link (jobs,link,firms) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
#
# # #
# create table xian_Account_link(
