...
def get_first_page():
    ...
    driver.find_element_by_xpath('//*[@id="kwdselectid"]').send_keys("java")  #可以针对其他岗位进行统计分析
    # 还要剔除本地选项　　（“”）
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into xian_java_link (jobs,link,firms) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
