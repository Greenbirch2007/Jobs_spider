...
def get_first_page():
    ...
    driver.find_element_by_xpath('//*[@id="kwdselectid"]').send_keys("税务")  #可以针对其他岗位进行统计分析
    # 还要剔除本地选项　　（“”）
...
def next_page():
    for i in range(1,43):  # selenium 循环翻页成功！
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into xian_Tax_link (jobs,link,firms) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
