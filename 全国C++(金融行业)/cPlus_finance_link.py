...
def next_page():
    for i in range(1,27):  # selenium 循环翻页成功！
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into cPlus_finance_links (jobs,link,firms) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
# #
# create table cPlus_finance_links(
