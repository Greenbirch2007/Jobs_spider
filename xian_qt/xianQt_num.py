...
def Python_sel_Mysql():
    ...
    for i in range(1,125):
        sql = 'select link from xian_Qt_link where id = %s ' % i
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into xian_Qt_num (jobs, salary,firms, nums0) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
