...
def get_first_page():
    ...
    time.sleep(0.5)
    driver.find_element_by_xpath('//*[@id="kwdselectid"]').send_keys("量化")  #可以针对其他岗位进行统计分析
    # 还要剔除本地选项　　（“”）
    # 剔除本地，选择全国范围
    ...
    driver.find_element_by_xpath('//*[@id="indtype_input"]').click()
    driver.find_element_by_xpath('//*[@id="indtype_click_center_left_each_41"]').click()
    driver.find_element_by_xpath('//*[@id="indtype_click_center_right_list_category_41_03"]').click()
    driver.find_element_by_xpath('//*[@id="indtype_click_bottom_save"]').click()
    driver.find_element_by_xpath('/html/body/div[2]/form/div/div[1]/button').click()   #搜索按键
...
def next_page():
    for i in range(1,84):  # selenium 循环翻页成功！
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into LH_finance_links (jobs,link,firms) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
